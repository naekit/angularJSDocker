				...
				function (
					...
					function computeCachedCssStyles(
						...
						if (!timings) {
							...
							if (
								timings.animationIterationCount === "infinite"
							) {
					...
					function computeCachedCssStaggerStyles(
						...
						if (gcsLookup.count(cacheKey) > 0) {
							...
							if (!stagger) {
								var staggerClassName = pendClasses(
									className,
									"-stagger"
								)
					...
					function computeTimings(node, className, cacheKey) {
						...
						timings.maxDelay =
							aD && tD ? Math.max(aD, tD) : aD || tD
						timings.maxDuration = Math.max(
							timings.animationDuration *
								timings.animationIterationCount,
					...
					return function init(element, initialOptions) {
						...
						var hasToStyles =
							styles.to && Object.keys(styles.to).length > 0
						...
						if (options.duration >= 0) {
							applyOnlyDuration =
								node.style[TRANSITION_PROP].length > 0
						...
						if (options.delay != null) {
							...
							if (flags.applyTransitionDelay) {
								temporaryStyles.push(
									getCssDelayStyle(delayStyle)
								)
							}

							if (flags.applyAnimationDelay) {
								temporaryStyles.push(
									getCssDelayStyle(delayStyle, true)
								)
							}
						}
						...
						function close(rejected) {
							...
							var animationTimerData =
								element.data(ANIMATE_TIMER_KEY)
						...
						function onAnimationProgress(event) {
							...
							if (
								Math.max(timeStamp - startTime, 0) >=
									maxDelayTime &&
						...
						function start() {
							...
							if (maxStagger) {
								$timeout(
									...
									Math.floor(
										maxStagger * itemIndex * ONE_SECOND
									),
									false
								)
							} else {
							...
							function triggerAnimationStart() {
								...
								var timerTime =
									maxDelayTime +
									CLOSING_TIME_BUFFER * maxDurationTime
								...
								if (animationsData.length) {
									...
									setupFallbackTimer =
										endTime >
										currentTimerData.expectedEndTime
			...
			this.$get = [
				...
				function (
					...
					var rootBodyElement = jqLite(
						...
						isDocumentFragment(rootNode) ||
							bodyNode.contains(rootNode)
							? rootNode
							: bodyNode
					)
					...
					function prepareAnchoredAnimation(
						...
						var clone = jqLite(
							getDomNode(outAnchor).cloneNode(true)
						)
						var startingClasses = filterCssClasses(
							getClassVal(clone)
						)
						...
						function calculateAnchorStyles(anchor) {
							...
							var coords =
								getDomNode(anchor).getBoundingClientRect()
						...
						function prepareInAnimation() {
							var endingClasses = filterCssClasses(
								getClassVal(inAnchor)
							)
							var toAdd = getUniqueValues(
								endingClasses,
								startingClasses
							)
							var toRemove = getUniqueValues(
								startingClasses,
								endingClasses
							)
							...
							var animator = $animateCss(clone, {
								...
								removeClass:
									NG_OUT_ANCHOR_CLASS_NAME + " " + toRemove,
								delay: true,
							})
				...
				function ($injector, $$AnimateRunner, $$jqLite) {
					...
					function lookupAnimations(classes) {
						...
						for (var i = 0; i < classes.length; i++) {
							var klass = classes[i],
								animationFactory =
									$animateProvider.$$registeredAnimations[
										klass
									]
				...
				function ($$animateJs, $$AnimateRunner) {
					return function initDriverFn(animationDetails) {
						if (animationDetails.from && animationDetails.to) {
							var fromAnimation = prepareAnimation(
								animationDetails.from
							)
							var toAnimation = prepareAnimation(
								animationDetails.to
							)
		...
		function ($animateProvider) {
			...
			function hasMatchingClasses(newClassString, currentClassString) {
				if (newClassString && currentClassString) {
					var currentClassMap =
						makeTruthyCssClassMap(currentClassString)
			...
			this.$get = [
				...
				function (
					...
					function queueAnimation(element, event, initialOptions) {
						...
						var isStructural =
							["enter", "move", "leave"].indexOf(event) >= 0
							...
							if (cancelAnimationFlag) {
								...
								if (joinAnimationFlag) {
									if (
										...
										event = newAnimation.event =
											existingAnimation.event
						...
						if (!isValidAnimation) {
							...
							isValidAnimation =
								(newAnimation.event === "animate" &&
									Object.keys(newAnimation.options.to || {})
										.length > 0) ||
								hasAnimationClasses(newAnimation)
						}
					...
					function closeChildAnimations(element) {
						...
						forEach(children, function (child) {
							...
							var animationDetails =
								activeAnimationsLookup.get(child)
							if (animationDetails) {
								switch (state) {
									case RUNNING_STATE:
					...
					function areAnimationsAllowed(
						...
						var rootElementDetected = isMatchingElement(
							element,
							$rootElement
						)
						...
						while (parentElement && parentElement.length) {
							...
							var details =
								activeAnimationsLookup.get(parentNode) || {}
							...
							if (!rootElementDetected) {
								...
								parentHost =
									parentElement.data(NG_ANIMATE_PIN_DATA)
								if (parentHost) {
					...
					return function (element, event, options) {
						...
						function getAnchorNodes(node) {
							...
							forEach(items, function (node) {
								var attr =
									node.getAttribute(NG_ANIMATE_REF_ATTR)
								if (attr && attr.length) {
									anchors.push(node)
								}
							})
